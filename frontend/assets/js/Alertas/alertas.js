(() => {
    ...
    <div class="col-md-6 col-lg-4 mb-4">
      <div class="card h-100 shadow border-0">
        <div class="card-header bg-light d-flex justify-content-between">
          ...
          <small>
            Puntaje:
            <strong>${safe(a.puntaje_maximo)}</strong>
          </small>
        </div>
        ...
        <div class="card-footer bg-white border-0">
          ...
          <div class="mt-2">
            <button
              ...
              data-actividad-id="${safe(a.actividad_id)}"
            >
              Ver
            </button>
          </div>
  ...
  function configurarBotonesVer() {
    document.querySelectorAll('.btn-ver').forEach(btn => {
      ...
      btn.addEventListener('click', function () {
        ...
        const actividadId = toInt(
          this.dataset.actividadId,
          0
        );

        if (!actividadId) {
          console.error('No se encontró actividad_id');
          alert('No se encontró la actividad seleccionada.');
          return;
        }

        /*
         * Guardamos la actividad seleccionada
         * para que actividades.html pueda recuperarla.
         */
        sessionStorage.setItem(
          'actividadIdRedireccion',
          String(actividadId)
        );

        /*
         * Redirección al módulo Actividades
         */
        window.location.href =
          'http://localhost:8001/pages/tareas/actividades.html';
      });
  ...
  function render(data, permitidas) {
    ...
    data.forEach(grupo => {
      ...
      const actividades =
        Array.isArray(grupo.actividades)
          ? grupo.actividades
          : [];
